...
class EmailSender {
    ...
    sendMail(from, to, subject, tplName, locals, attachments, subDir) {
        let self = this;
        let d = Q.defer();
        ...
        if (!fs.existsSync(path.resolve(templatePath, tplName))) {
            ...
            let platformApp = 'firefox';
            if (process.platform === 'win32')
                platformApp = 'chrome';
            else if (process.platform === 'darwin')
                platformApp = 'google chrome';

            const email = new Email({
                transport: self.config.smtp,
                views: {
                    root: templatePath
                },
                preview: {
    ...
    sendSimpleMail(from, to, subject, bodyType, body, attachments) {
        let self = this;
        let d = Q.defer();
